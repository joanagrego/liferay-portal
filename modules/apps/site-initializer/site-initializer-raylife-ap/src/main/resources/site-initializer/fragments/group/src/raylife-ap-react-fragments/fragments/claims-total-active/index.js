...
export default function () {
	...
	useEffect(() => {
		Promise.allSettled([
			...
			const [productQuotesResult, claimsResult, policiesResult] = results;

			const columnsArr = [];
			const colorsObj = {};

			const activeClaims = claimsResult?.value?.data;

			const totalPolicies = policiesResult?.value?.data?.items;

			const productNameOfClaims = activeClaims?.items?.map((claim) => {
			...
			setChartTitle(activeClaims?.totalCount);
	...
	return (
		<div className="claims-total-active-container d-flex flex-column flex-shrink-0 pb-4 pt-3 px-3">
			<div className="claims-total-active-title font-weight-bolder h4">
				Total Active
			</div>
